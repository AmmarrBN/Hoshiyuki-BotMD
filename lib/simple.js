    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                if (fileSize >= 20000) throw new Error(' Ã¢Å“Â³Ã¯Â¸  El tamaÃƒÂ±o del archivo es demasiado grande\n\n')
                let opt = {}
...
END:VCARD
        ...
        sendButton: {
             ...
             */
                ...
                async value(jid, head = '', body = '', footer = '', json) {
				const template = [
				  {
				    "name": "quick_reply",
				...
				if (Array.isArray(json[0])) {
				    json.forEach(item => {
				      ...
				      newObj.buttonParamsJson = newObj.buttonParamsJson.replace(/dispText/g, item[0]);
				      newObj.buttonParamsJson = newObj.buttonParamsJson.replace(/idText/g, item[1]);
				      buttons.push(newObj);
				    });
				  } else {
				    const newObj = JSON.parse(JSON.stringify(template[0]));
				    newObj.buttonParamsJson = newObj.buttonParamsJson.replace(/dispText/g, json[0]);
				    newObj.buttonParamsJson = newObj.buttonParamsJson.replace(/idText/g, json[1]);
				    buttons.push(newObj);
				  }
        ...
        sendImgButton: {
             ...
             */
                ...
                async value(jid, buffer, head = '', body = '', footer = '', json) {
				const template = [
				  {
				    "name": "quick_reply",
				    "buttonParamsJson": "{\"display_text\":\"dispText\",\"id\":\"idText\"}"
				  ...
				  viewOnceMessage: {
				    message: {
				        ...
				        interactiveMessage: proto.Message.InteractiveMessage.create({
				          ...
				          header: proto.Message.InteractiveMessage.Header.create({
				            ...(await prepareWAMessageMedia({ image: { url: buffer } }, { upload: conn.waUploadToServer })),
				            title: head,
				            subtitle: head,
				            hasMediaAttachment: false
				          }),
        ...
        //---
        sendList: {
        async value(jid, head, body, footer, title, description, json, options) {
        ...
        //--
        sendImgList: {
async value(jid, buffer, head, body, footer, title, description, json, options) {
           ...
           if (Array.isArray(json[0])) {
               json.forEach(item => {
                ...
                const messageButtons = {"name": "single_select","buttonParamsJson": JSON.stringify({"title": title, "sections": [{ "title": description, "rows": buttons }] })}
                const urlButtons = {"name": "cta_url", "buttonParamsJson": JSON.stringify({"display_text": "Channel Hoshiyuki", "url":"https://whatsapp.com/channel/0029VaWNXqF8fewpBpaCiQ1K", "merchant_url": "https://whatsapp.com/channel/0029VaWNXqF8fewpBpaCiQ1K"})}
				    ...
				    viewOnceMessage: {
				        message: {
				            ...
				            interactiveMessage: proto.Message.InteractiveMessage.create({
				                ...
				                nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
				                    buttons: [messageButtons, urlButtons]
				                })
				            })
				          }
				        }
